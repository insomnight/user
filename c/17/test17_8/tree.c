...
bool AddItem(const Item * pi, Tree * ptree)
{
    ...
    if ((root = SeekItem(pi, ptree).child) != NULL)
    {
        if(root->item.count < SIZE)
        {
            strcpy(root->item.petkind[root->item.count],pi->petkind[0]);
            root->item.count++;
            ptree->size++;
            return true;/*提前返回*/
        }else
...
bool DeleteItem(const Item * pi, Tree * ptree)
{
    ...
    ptree->size -= look.child->item.count;
...
static void AddNode(Trnode * new_node, Trnode * root)
{
    ...
    else                                    /*重复项把类别添加到数组*/
    {
        if(root->item.count < SIZE)
        {
            strcpy(root->item.petkind[root->item.count],new_node->item.petkind[0]);
            root->item.count++;
        }
    }
}
