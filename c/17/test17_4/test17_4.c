...
int main(void)
{
    ...
    for(int i = 0; i < MALLCOUNT; i++)
    {
        ...
        printf("Case Study: Sigmund Lander's Advice Booth %d:\n",i);
        printf("Enter the number of mall %d simulation hours:\n",i);
        scanf("%d", &hours[i]);
        cyclelimit[i] = MIN_PER_HR * hours[i];
        printf("Enter the average number of mall %d customers per hour:\n",i);
        scanf("%d", &perhour[i]);
